...
namespace cavc
{
template <typename Real, std::size_t NodeSize = 16>
...
public:
    ...
    void finish()
    {
        ...
        for (std::size_t i = 0; i < m_numItems; ++i)
        {
            ...
            // [0 -> n - 1] such that the min of the entire set of bounding boxes maps to 0 and the
            // max of the entire set of bounding boxes maps to n - 1 our 2d space is x: [0 -> n-1]
            // and y: [0 -> n-1], our 1d hilbert curve value space is d: [0 -> n^2 - 1]
    ...
    template <typename F>
    void visitBoundingBoxes(F &&visitor) const
    {
        ...
        while (!done)
        {
            ...
            for (std::size_t pos = nodeIndex; pos < end; pos += 4)
            {
                ...
                if (!visitor(level, m_boxes[pos], m_boxes[pos + 1], m_boxes[pos + 2],
                             m_boxes[pos + 3]))
                {
                    return;
                }
    ...
    // false is returned.
    template <typename F>
    void visitItemBoxes(F &&visitor) const
    {
        for (std::size_t i = 0; i < m_levelBounds[0]; i += 4)
        {
            if (!visitor(m_indices[i >> 2], m_boxes[i], m_boxes[i + 1], m_boxes[i + 2],
                         m_boxes[i + 3]))
            {
                return;
            }
    ...
    // Query the spatial index, invoking a visitor function for each index that overlaps the
    // bounding box given. Visitor function has the signature bool(std::size_t index), if visitor
    // returns false the query stops early, otherwise the query continues. This overload accepts an
    // existing vector to use as a stack and takes care of clearing the stack before use.
    template <typename F>
    void visitQuery(Real minX, Real minY, Real maxX, Real maxY, F &&visitor,
                    std::vector<std::size_t> &stack) const
    {
        CAVC_ASSERT(m_pos == 4 * m_numNodes,
                    "data not yet indexed - call Finish() before querying");
