...
constexpr inline cavc_real TEST_EPSILON()
{
    return 1e-5;
}
...
struct PolylineProperties
{
    ...
    PolylineProperties(std::size_t vertexCount, cavc_real area, cavc_real pathLength,
                       cavc_real minX, cavc_real minY, cavc_real maxX, cavc_real maxY) :
        vertexCount(vertexCount),
        area(area), pathLength(pathLength), minX(minX), minY(minY), maxX(maxX), maxY(maxY)
    {
    }
