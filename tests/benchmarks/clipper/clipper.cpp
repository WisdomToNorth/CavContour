...
{
    double a =
        math.atan2(m_sinA, m_normals[k].X * m_normals[j].X + m_normals[k].Y * m_normals[j].Y);
...
MathKernel::MathKernel() :
    cos{[](double v) { return std::cos(v); }}, acos{[](double v) { return std::acos(v); }},
    sin{[](double v) { return std::sin(v); }}, tan{[](double v) { return std::tan(v); }},
