...
extern "C"
{
    ...
    // Create/alloc a new cavc_pline initialized with data given. vertex_data may be null
    // (vertex_count will still be used to set capacity), if is_closed is 0 then polyline will be
    // open, otherwise it will be closed.
    CAVC_API cavc_pline *cavc_pline_new(cavc_vertex const *vertex_data, uint32_t vertex_count,
                                        int is_closed);

    // Delete/free a cavc_pline that was created with cavc_pline_new or released from a
    // cavc_pline_list. NOTE: Do not call this function on cavc_pline's that are owned by a
    // cavc_pline_list!
    ...
    // Sets the vertex data of a cavc_pline. Must pass in vertex_count to indicate how many vertexes
    // to copy from the vertex_data array.
    ...
    // Sets the polyline to be closed or open, if is_closed is 0 then polyline will be open,
    ...
    // the result, option_flags are bit flags that allow for indicating information about the
    // polyline or forcing certain behaviors in the offset generation, more flags may be added in
    // future versions. If no flags are set then the polyline is assumed to have no self intersects.
    ...
    // remaining is filled with the closed polylines that remain after combining, subtracted is
    // filled with closed polylines that represent subtracted space (in the case of islands after a
    // union or exclude). If pline_a or pline_b is an open polyline or has self intersects then the
    // result is undefined.
    ...
    // Returns the path length of the cavc_pline given. If pline vertex count is less than 2 then 0
    // is returned.
    CAVC_API cavc_real cavc_get_path_length(cavc_pline const *pline);

    // Returns the signed area of the cavc_pline given. If pline is open or vertex count is less
    // than 2 then 0 is returned. If pline goes clockwise then a negative signed area is returned,
    ...
    // Compute the axis aligned extents of the pline, results are written to min_x, min_y, max_x,
    // and max_y output parameters. If pline is empty then min_x and min_y are filled with positive
    // infinity and max_x and max_y are filled with negative infinity.
    CAVC_API void cavc_get_extents(cavc_pline const *pline, cavc_real *min_x, cavc_real *min_y,
                                   cavc_real *max_x, cavc_real *max_y);

    // Finds the closest point on the polyline to the point given. closest_start_index is filled
    // with the starting vertex index of the segment that the closest point is on. closest_point is
    // filled with the closest point on the polyline. distance is filled with the distance from the
    // point given to the closest point. pline must not be empty.
